...
jQuery(function($){
	...
	$.post(_path+"/xsxk/zzxkyzb_cxZzxkYzbChoosedDisplay.html",{
		...
		"zyfx_id":$("#zyfx_id").val(),"bh_id":$("#bh_id").val(),"xz":$("#xz").val(),"ccdm":$("#ccdm").val(),
		...
		if(data!=null && data.length>0){
			...
			for(var i=0; i<data.length; i++){
				...
				if(rightKchId!=modelA.t_kch_id){
					...
					s_html.push(modelA.kcmc+" - "+modelA.xf+" "+$.i18n.get("msg_xf")+"<span class='pull-right'></span></h6>");
	...
	$(document).off("mousedown",'button.btn-tk').on("mousedown",'button.btn-tk',function(event){
		...
		$.post(_path+"/xsxk/zzxkyzb_xkJcXgqzqxZzxkYzb.html",{
			...
			if(data=="2"){
				...
				jQuery.showDialog(_path +'/xsxk/zzxkyzb_xkZzxkYzbQztx.html?jxb_id='+do_jxb_id+'&syqz='+syqz,ckmc,{
					width:"350px",
					modalName:"addModal",
					buttons:{
						success : {
...
function getZjxbByIds(jxb_ids){
	...
	$.post(_path+"/xsxk/zzxkyzb_cxZkcZzxkYzb.html",
		{jxb_ids:jxb_ids,xkkz_id:$("#xkkz_id").val(),bklx_id:$("#bklx_id").val(),kklxdm:$("#kklxdm").val(),rlkz:$("#rlkz").val(),zyh_id:$("#zyh_id").val(),njdm_id:$("#njdm_id").val()},
		function(data){
			if(data!=null && data!="0"){
				var tmp_html = "";
				for(var i=0; i<data.length; i++){
					...
					tmp_html = tmp_html + "' data-container='body' data-toggle='popover' data-placement='auto' data-content='"+$.i18n.get("msg_sksjdd")+"："+data[i].sksj+" / "+data[i].jxdd+"'><input type='hidden' name='zkc_jxb_id' value='"+data[i].do_jxb_id+"'/></a>";
...
function checkCourse_20(trObj,jxb_id,do_jxb_id,kch_id,jxbzls,qz){//检测冲突
	if($("#sfyxsksjct").val()=="1"||$("#tbtkxqxktskg").val()=="1"){
		$.ajaxSetup({async:false});
...
function checkCourse_22(trObj,jxb_id,do_jxb_id,kch_id,jxbzls,qz){
	...
	if($("#xksdxjckg").val()=="1"){
		$.ajaxSetup({async: false});
		$.post(_path + "/xsxk/zzxkyzbjk_cxCheckJckg.html", {
			"jxb_id": jxb_id, "xkkz_id" : $("#xkkz_id").val()
		}, function (data) {
			if (data=='1') {
				$.showDialog(_path + "/xsxk/zzxkyzbjk_cxXsxzjcView.html", "选择教材",{
					width:"900px",
					modalName:"xzjcModal",
...
function saveDjxbCourseBc(trObj,kch_id,jxb_id,do_jxb_id,jxbzls,jxb_arr,jcxx_arr){
	...
	$.post(_path+"/xsxk/zzxkyzbjk_xkBcZyZzxkYzb.html",{
		...
		rlkz:rlkz,rlzlkz:rlzlkz,sxbj:sxbj,xxkbj:xxkbj,cxbj:$("#cxbj_"+kch_id).val(),kklxdm:$("#kklxdm").val(),xkkz_id:$("#xkkz_id").val(),
		njdm_id:$("#njdm_id").val(),zyh_id:$("#zyh_id").val(),xklc:$("#xklc").val(),xkxnm:$("#xkxnm").val(),xkxqm:$("#xkxqm").val(),qz:$("#qz").val(),"jcxx_id":jcxx_arr.join(',')
	},function(data){
		setTimeout(function(){
			if(data!=null){
				var flag = data.flag;
...
function refreshDataAddZzxk(trObj,jxb_id,do_jxb_id,kch_id,jxbzls,sxbj,qz){
	...
	if(len==0){
		...
		add_html.push("<td class='h_time'>"+$.i18n.get("msg_sksj")+"</td><td class='h_addr'>"+$.i18n.get("msg_jxdd")+"</td>");
		if($("#sfxskssj").val()=="1"){
	...
	if(jxbzls>1){
		...
		$(currentTdObj).find("li").each(function(index,item){
			...
			add_html.push("' data-container='body' data-toggle='popover' data-placement='auto' data-content='"+$.i18n.get("msg_sksjdd")+"："+s_sksj+" / "+s_jxdd+"'><input type='hidden' name='zkc_jxb_id' value='"+s_do_jxb_id+"'/></a>");
		});
	}else{
...
function orderBook(jxb_id,sfydjc){
	...
	$.ajaxSetup({async: false});
	$.post(_path + "/xsxk/zzxkyzbjk_cxUpdateJcydbj.html", {
